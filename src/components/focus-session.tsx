...
export function FocusSession({
  ...
  return (
    <section
      ...
      className="app-surface glow-card rounded-[2rem] border-2 border-[var(--sky-500)]/30 p-4 sm:p-5"
    >
      ...
      <div className="mt-4 rounded-[1.4rem] border border-[var(--line)] bg-[var(--glass-bg)] p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div aria-live="polite" className="min-w-0">
            ...
            <h4 className="mt-1 text-xl font-semibold text-[var(--ink-950)]">
            ...
            {nextOccurrence ? (
              <p className="mt-1 text-xs text-[var(--ink-500)]">
                Ensuite : <span className="font-semibold text-[var(--ink-700)]">{nextOccurrence.taskTemplate.title}</span>
                {nextOccurrence.taskTemplate.estimatedMinutes ? ` · ${nextOccurrence.taskTemplate.estimatedMinutes} min` : ""}
              </p>
            ) : null}
          </div>

          <div className="rounded-[1.2rem] border border-[var(--line)] bg-[rgba(47,109,136,0.08)] px-4 py-2.5 text-center">
            <p className="text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-[var(--ink-500)]">
              Temps réel
            </p>
            <p className="mt-0.5 text-2xl font-semibold text-[var(--ink-950)]">{formatElapsed(elapsedMs)}</p>
          </div>
        </div>
