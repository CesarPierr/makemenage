          ...
          return (
            <div key={col.id} className="app-surface rounded-[1.8rem] p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="ml-auto text-xs font-semibold text-[var(--ink-500)]">
                  {items.length}
                </span>
              </div>
              <ul aria-label={`Tâches ${col.label}`} className="space-y-2">
                {items.length === 0 ? (
                  <li className="rounded-xl border border-dashed border-[var(--line)] p-3 text-center text-xs text-[var(--ink-500)]">
                    Aucune
                  </li>
                ) : (
                  items.map((o) => (
                    <li
                      key={o.id}
                      className="rounded-xl border border-[var(--line)] px-3 py-2.5 text-xs"
                    ...
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-1 flex flex-wrap gap-1 text-[0.6rem] text-[var(--ink-500)]">
                          ...
                          {o.assignedMember ? (
                            <div className="mt-1 flex items-center gap-1">
                              ...
                              <span className="text-[0.6rem] text-[var(--ink-500)]">{o.assignedMember.displayName}</span>
                            </div>
                          ) : null}
                        </div>
                      </div>
              ...
              <ul className="space-y-1.5">
                {items.slice(0, 4).map((o) => (
                  <li key={o.id} className="flex items-center gap-2 rounded-xl px-2.5 py-2 text-xs" style={{ background: col.surface }}>
                    ...
                    <span className="shrink-0 text-[0.6rem] text-[var(--ink-500)]">
                      {format(new Date(o.scheduledDate), "EEE", { locale: fr })}
                    </span>
                  </li>
                ))}
                {items.length > 4 ? (
                  <li className="text-center text-[0.6rem] text-[var(--ink-500)] py-1">+{items.length - 4} de plus</li>
                ) : null}
              </ul>
            </div>
          );
