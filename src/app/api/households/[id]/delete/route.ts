...
export async function POST(request: Request, { params }: Params) {
  ...
  if (!confirmed) {
    return redirectTo(request, `/app/settings?household=${id}&panel=danger&delete=confirm_required`);
  }
  ...
  return redirectTo(request, `/app/settings?household=${id}&panel=danger&delete=${result.status}`);
}
