...
src/
  app/
    app/              authenticated UI (App Router segments)
      ...
      calendar/
      history/
      settings/       6 sub-routes: team, access, planning, danger, households, integrations
    api/              JSON endpoints (households, tasks, occurrences, members, auth, ...)
    page.tsx          public landing
  components/
    ui/               primitives: dialog, bottom-sheet, toast
    occurrence-card.tsx       core interaction surface
    task-creation-wizard.tsx  multi-step task builder
    app-shell.tsx             top header + bottom nav (4 items + "Plus" sheet)
  lib/
...
7. **No file > 500 lines** without a clear reason. `task-creation-wizard.tsx` (615) is the next split target — don't grow it further.
...
- **Tailwind 4** has no `tailwind.config.js`. Custom tokens live in `src/app/globals.css` under `@theme`. Don't try to add a config file.
...
- ✅ Mobile UX foundation shipped — bottom sheet, compact cards, toast system, `useFormAction`, PWA, settings sub-routes, onboarding wizard, calendar agenda mobile.
- 🟡 Engagement in progress — optimistic UI on complete/skip, streak UI, comments on occurrences.
- 🟡 Hardening partial — rate limiting and CSP done; CSRF, pagination, structured logging, dark mode, a11y pass remain.
